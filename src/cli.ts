...
program
    ...
    .option('-a, --agent <agents...>', 'Specify agents to install to (use --list-agents to see available agents)')
    ...
    .configureOutput({
        outputError: (str, write) => {
            if (str.includes('missing required argument')) {
                ...
                console.log(`    ${chalk.cyan('npx unifai')} ${chalk.yellow('<source>')} ${chalk.dim('[options]')}`);
                console.log();
                console.log(chalk.dim('  Example:'));
                console.log(`    ${chalk.cyan('npx unifai')} ${chalk.yellow('vercel-labs/agent-skills')}`);
                console.log();
                console.log(chalk.dim('  Run') + ` ${chalk.cyan('npx unifai --help')} ` + chalk.dim('for more information.'));
                console.log();
            } else {
                write(str);
            }
...
async function main(source: string, options: Options) {
    console.log();
    p.intro(chalk.bgCyan.black(' unifai '));
